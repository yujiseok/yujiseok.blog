...
페이지를 새로고침하지 않고 상태를 유지할 수 있습니다. `popstate` 이벤트를 통해 브라우저의 뒤로/앞으로 가기 동작에 대응할 수 있습니다.

- 주요 메서드
  - `pushState(state, title, url)`: 현재 페이지를 새로고침하지 않고 새로운 히스토리 항목을 추가합니다.
    - state: 해당 항목과 연관된 상태 객체
    - title: 대부분의 브라우저에서는 현재 무시됨
    - url: 주소 표시줄에 나타날 URL
  - `replaceState(state, title, url)`:
    현재 히스토리 항목을 새로운 데이터로 대체합니다. (새 항목 생성 x)
  - `back()`:
