...
현재 저는 `13.2.4` 버전을 사용 중입니다. Next 13 버전의 app 디렉터리를 사용하려면 아래와 같이 터미널에 입력합니다.
...
저는 다른 데이터베이스나, HeadlessCMS를 사용하지 않고 제 로컬에 **mdx 파일**들을 관리하기로 하였습니다. 이때 효율적으로 mdx 파일들을 관리하기 위해 [contentlayer](https://www.contentlayer.dev/)를 활용합니다.
`contentlayer`는 content의 형태를 **type-safe**한 JSON 데이터로 변환해 주고 content를 관리해 주는 SDK입니다.

contentlayer를 사용하기 위해선, 몇가지 과정이 필요합니다.

1. contentlayer를 설치하기 위해 아래와 같은 명령어를 입력합니다.
...
저는 최신순으로 포스트를 정렬하기 위해 `.sort` 메서드를 사용해 내림차순 정렬해 줍니다.
