...
Next.js는 흑마법을 🔮 부려 기본적으로 페이지를 정적으로 만드는데 실제 서비스에선 모든 페이지가 동적으로 생성되고 있었습니다.
...
하지만 모든 페이지를 동적으로 생성하면 Next.js의 풀 라우트 캐시(Full Route Cache)를 활용하지 못하고 무시하게 됩니다.
이로 인해 서버는 이미 만들어진 결과물을 재사용하는 대신 사용자의 요청마다 HTML과 데이터(RSC Payload)를 매번 새롭게 생성해야 합니다.
결과적으로 서버 과부하 및 비용 증가, 페이지 깜빡임과 느린 페이지 전환, 클라이언트 상태 유실로 인한 유저 경험 저하 등 비효율적인 상황을 초래합니다.
...
- **모든 `fetch` 요청 캐시 무시:** 페이지 내의 특별한 설정이 존재하지 않는 `fetch` 요청은 `{ cache: 'no-store' }`로 처리되는 것과 동일합니다. 개별 `fetch` 요청에 `cache` 옵션을 설정하면 해당 캐시 옵션이 우선됩니다.
...
이 설정은 페이지의 **기본 동작**을 동적으로 만들 뿐, 개별 `fetch` 요청에 설정된 고유한 캐시 설정을 무시하지 않습니다.
...
`dynamic = 'force-dynamic'`: 풀 라우트 캐시 무시 ✅

이 설정은 더 강력하고 포괄적인 규칙입니다. 라우트 전체에 "풀 라우트 캐시를 허용하지 않고 무조건 동적으로만 동작해"라는 명령과 같습니다.

그 결과, 개별 `fetch`에 명시적인 캐시 옵션이 없다면 `{ cache: 'no-store' }`가 기본값으로 강제되지만, 직접 설정한 옵션이 있다면 그 설정이 우선 적용됩니다.
...
### 6. 데이터 캐시 vs 풀 라우트 캐시

Next.js의 캐싱은 크게 두 가지로 나뉩니다.

- **데이터 캐시 (Data Cache)**: `fetch`를 통해 가져온 **개별 데이터**를 캐싱 합니다. 요청 간에 공유됩니다.
- **풀 라우트 캐시 (Full Route Cache)**: 렌더링 된 페이지와 RSC 페이로드를 캐싱 합니다. 이 덕분에 정적 페이지(SSG/ISR)는 매우 빠르게 로드될 수 있습니다. 또한 페이지 이동 시 RSC 페이로드를 사용하여 빠르게 탐색할 수 있습니다.
...
- **SSG / ISR (정적 페이지)**: **두 캐시 모두 사용 ✅**
  - `fetch`로 데이터를 가져와 **데이터 캐시**에 저장하고, 그 데이터로 렌더링 한 페이지를 **풀 라우트 캐시**에 저장합니다.
- **SSR (동적 페이지)**: **데이터 캐시만 사용 ✅**
  - 페이지를 요청마다 새로 렌더링 하므로 **풀 라우트 캐시**는 사용하지 않습니다.
...
- `fetch(..., { cache: 'no-store' })`: 상향식(Bottom-up) 결정
  ...
  - 이 데이터의 동적인 특성 때문에, 해당 데이터를 사용하는 페이지 전체가 자동으로 동적 렌더링으로 전환됩니다.
...
- `dynamic = 'force-dynamic'`: 하향식(Top-down) 결정
  - 페이지 전체를 무조건 동적으로 렌더링하라고 명령하는 설정입니다. (풀 라우트 캐시 비활성화)
  - 이 라우트 세그먼트는 그 하위 **모든 `fetch` 요청에 전파**되어, 별도의 설정이 없다면 `{ cache: 'no-store' }`가 기본값으로 강제됩니다.
  - 하지만, 명시적인 캐시 설정이 있다면 그 설정이 우선 적용됩니다.
...
- **`export const dynamic = 'force-dynamic'` 사용 시:**
  ...
  2. 이 설정은 페이지 내 fetch 요청들의 **기본값(Default)**을 `{ cache: 'no-store' }`로 변경합니다. 옵션을 생략한 `fetch`들은 캐싱되지 않습니다.
  3. 개발자의 의도에 따른 명시적인 설정이 있다면 그 설정이 우선 적용됩니다.
...
> 변경되는 버전에 대해 항상 확인하고 학습하는 습관을 가져야겠다고 느꼈습니다.
